...
sstHud::Draw( vsRenderQueue *queue )
{
	...
	vsTransform2D t;
	t.SetTranslation(rightEdge - 10.0f,-440.0f);
...
//	list->LineTo( vsVector2D(1000.0f, -300.0f) );
	...
	t.SetTranslation(leftEdge + 10.0f,-440.0f);
	list->PushTransform( t );
	...
	if ( m_intermissionBrightness )
	{
		...
		t.SetTranslation(0.f,0.f);
		list->PushTransform(t);
		list->SetColor( vsColor(0.0f, 0.0f, 1.0f, m_intermissionBrightness) );
		list->Append( *hits );
		list->PopTransform();

		t.SetTranslation(0.f,50.f);
		list->PushTransform(t);
		list->SetColor( vsColor(0.0f, 0.0f, 1.0f, m_intermissionBrightness) );
		list->Append( *acc );
		list->PopTransform();
